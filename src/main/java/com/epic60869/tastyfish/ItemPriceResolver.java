...
public final class ItemPriceResolver {
    ...
    private static final String LBIN_URL="https://moulberry.codes/lowestbin.json";
    ...
    private static void refresh(boolean force){
        ...
        CompletableFuture<HttpResponse<String>> items=fetch(ITEMS_URL);
        CompletableFuture<HttpResponse<String>> bazaar=fetch(BAZAAR_URL);
        CompletableFuture<HttpResponse<String>> bin=fetch(LBIN_URL);

        items.whenComplete((r,e)->{
            if(e==null){try{
                String body=r.body();
                NPC.clear(); NPC.putAll(parseNpc(body));
                NAME_TO_ID.clear(); NAME_TO_ID.putAll(parseNames(body));
            }catch(Throwable t){System.err.println("[TastyFish] Hypixel item price parse failed: "+t);}}
        });
        bazaar.whenComplete((r,e)->{
        ...
        CompletableFuture.allOf(items.handle((r,e)->null),bazaar.handle((r,e)->null),bin.handle((r,e)->null))
