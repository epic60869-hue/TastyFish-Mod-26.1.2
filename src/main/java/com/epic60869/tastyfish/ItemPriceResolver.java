...
public final class ItemPriceResolver {
    ...
    private static final Map<String,Double> NPC=new HashMap<>(),BAZAAR=new HashMap<>(),LBIN=new HashMap<>();
    ...
    public static double value(String id){
        if(id==null||id.isBlank())return 0; refresh();
    ...
    public static double valueByName(String name){
        if(name==null||name.isBlank())return 0; refresh();
        String clean=cleanName(name); String id;
        synchronized(NAME_TO_ID){id=NAME_TO_ID.get(clean);}
        if(id!=null){double v=value(id);if(v>0)return v;}
        return value(clean.toUpperCase(Locale.ROOT).replace(' ','_'));
    }

    private static String cleanName(String s){return s.replaceAll("§[0-9a-fk-or]","").replaceAll("[✪★]+"," ").replaceAll("\\s+"," ").trim().toLowerCase(Locale.ROOT);}
    ...
    private static Map<String,Double> parseNpc(String body){Map<String,Double> o=new HashMap<>();JsonObject root=JsonParser.parseString(body).getAsJsonObject();if(!root.has("items"))return o;for(JsonElement e:root.getAsJsonArray("items")){if(!e.isJsonObject())continue;JsonObject i=e.getAsJsonObject();try{double p=i.get("npc_sell_price").getAsDouble();if(p>0)o.put(i.get("id").getAsString(),p);}catch(Exception ignored){}}return o;}
    private static Map<String,String> parseNames(String body){Map<String,String> o=new HashMap<>();JsonObject root=JsonParser.parseString(body).getAsJsonObject();if(!root.has("items"))return o;for(JsonElement e:root.getAsJsonArray("items")){if(!e.isJsonObject())continue;JsonObject i=e.getAsJsonObject();try{o.put(cleanName(i.get("name").getAsString()),i.get("id").getAsString());}catch(Exception ignored){}}return o;}
    private static Map<String,Double> parseBazaar(String body){Map<String,Double> o=new HashMap<>();JsonObject root=JsonParser.parseString(body).getAsJsonObject();if(!root.has("products"))return o;for(var e:root.getAsJsonObject("products").entrySet()){try{double p=e.getValue().getAsJsonObject().getAsJsonObject("quick_status").get("buyPrice").getAsDouble();if(p>0)o.put(e.getKey(),p);}catch(Exception ignored){}}return o;}
    private static Map<String,Double> parseBin(String body){Map<String,Double> o=new HashMap<>();JsonElement root=JsonParser.parseString(body);if(!root.isJsonObject())return o;for(var e:root.getAsJsonObject().entrySet()){try{double p=e.getValue().getAsDouble();if(p>0)o.put(e.getKey(),p);}catch(Exception ignored){}}return o;}
}
