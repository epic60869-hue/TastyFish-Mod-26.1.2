...
public final class TastyFishMod implements ClientModInitializer {
    ...
    @Override public void onInitializeClient(){
        ...
        ClientCommandRegistrationCallback.EVENT.register((d,r)->{
            d.register(ClientCommands.literal("tf").executes(c->{openMenu();return 1;}));
            d.register(ClientCommands.literal("tastyfish").executes(c->{openMenu();return 1;}));
        });
        System.out.println("[TastyFish] Standalone farming tracker loaded. SkySoft is not required.");
    }

    private void openMenu(){
        Minecraft.getInstance().setScreen(new TastyFishScreen(config));
    }
    ...
    private void tick(Minecraft mc){
        ...
        TastyFishStandalone.Snapshot s=TastyFishStandalone.get().snapshot();
        if(!s.valid())return;
        UUID uuid=mc.getUser().getProfileId();
        uploader.upload(config,mc.getUser().getName(),uuid==null?"":uuid.toString(),"",sessionId,s);
    }
}
