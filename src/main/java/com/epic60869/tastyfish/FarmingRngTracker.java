...
public final class FarmingRngTracker {
    ...
    private void handle(Component message){
        ...
        Parsed p=parse(raw);if(p==null)return;
        String key=p.name().toLowerCase(Locale.ROOT);
        long until=System.currentTimeMillis()+SHOW_MILLIS;
        Drop existing=activeDrops.get(key);
        if("Seasoning".equalsIgnoreCase(p.name())){
            int amount=(existing==null?0:existing.amount())+p.amount();
            activeDrops.put(key,new Drop(amount,p.name(),rarity(raw),-1L,until));
            return;
        }
        if(existing==null){
            activeDrops.putIfAbsent(key,new Drop(p.amount(),p.name(),rarity(raw),0L,until));
        }else{
            activeDrops.computeIfPresent(key,(ignored,current)->new Drop(current.amount()+p.amount(),current.name(),current.rarity(),current.unitPrice(),until));
        }
        ItemPriceResolver.valueByNameAsync(p.name()).thenAccept(price->
            activeDrops.computeIfPresent(key,(ignored,current)->new Drop(current.amount(),current.name(),current.rarity(),Math.round(price),current.shownUntil()))
        );
    }
